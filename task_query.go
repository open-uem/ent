...
func (_q *TaskQuery) QueryTags() *TagQuery {
...
func (_q *TaskQuery) QueryProfile() *ProfileQuery {
...
func (_q *TaskQuery) QueryReports() *TaskReportQuery {
...
func (_q *TaskQuery) WithTags(opts ...func(*TagQuery)) *TaskQuery {
	query := (&TagClient{config: _q.config}).Query()
...
func (_q *TaskQuery) WithProfile(opts ...func(*ProfileQuery)) *TaskQuery {
	query := (&ProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithReports(opts ...func(*TaskReportQuery)) *TaskQuery {
	query := (&TaskReportClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Task, init func(*Task), assign func(*Task, *Tag)) error {
	...
	for _, n := range neighbors {
		fk := n.task_tags
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_tags" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_tags" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TaskQuery) loadProfile(ctx context.Context, query *ProfileQuery, nodes []*Task, init func(*Task), assign func(*Task, *Profile)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i].profile_tasks == nil {
			continue
		}
		fk := *nodes[i].profile_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "profile_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadReports(ctx context.Context, query *TaskReportQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskReport)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
