...
func (_q *MetadataQuery) QueryOwner() *AgentQuery {
...
func (_q *MetadataQuery) QueryOrg() *OrgMetadataQuery {
...
func (_q *MetadataQuery) WithOwner(opts ...func(*AgentQuery)) *MetadataQuery {
	query := (&AgentClient{config: _q.config}).Query()
...
func (_q *MetadataQuery) WithOrg(opts ...func(*OrgMetadataQuery)) *MetadataQuery {
	query := (&OrgMetadataClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MetadataQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Metadata, error) {
	...
	if _q.withOwner != nil || _q.withOrg != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withOwner; query != nil {
		if err := _q.loadOwner(ctx, query, nodes, nil,
			func(n *Metadata, e *Agent) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *MetadataQuery) loadOwner(ctx context.Context, query *AgentQuery, nodes []*Metadata, init func(*Metadata), assign func(*Metadata, *Agent)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Metadata)
	for i := range nodes {
		if nodes[i].agent_metadata == nil {
			continue
		}
		fk := *nodes[i].agent_metadata
...
func (_q *MetadataQuery) loadOrg(ctx context.Context, query *OrgMetadataQuery, nodes []*Metadata, init func(*Metadata), assign func(*Metadata, *OrgMetadata)) error {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *MetadataSelect) Modify(modifiers ...func(s *sql.Selector)) *MetadataSelect {
