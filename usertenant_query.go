...
func (utq *UserTenantQuery) Clone() *UserTenantQuery {
	...
	return &UserTenantQuery{
		...
		sql:       utq.sql.Clone(),
		path:      utq.path,
		modifiers: append([]func(*sql.Selector){}, utq.modifiers...),
	}
}
...
func (utq *UserTenantQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*UserTenant, init func(*UserTenant), assign func(*UserTenant, *User)) error {
	...
	nodeids := make(map[string][]*UserTenant)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (utq *UserTenantQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*UserTenant, init func(*UserTenant), assign func(*UserTenant, *Tenant)) error {
	...
	nodeids := make(map[int][]*UserTenant)
	for i := range nodes {
		fk := nodes[i].TenantID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
...
func (utq *UserTenantQuery) Modify(modifiers ...func(s *sql.Selector)) *UserTenantSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (uts *UserTenantSelect) Modify(modifiers ...func(s *sql.Selector)) *UserTenantSelect {
