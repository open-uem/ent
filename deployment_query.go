...
func (_q *DeploymentQuery) QueryOwner() *AgentQuery {
...
func (_q *DeploymentQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *DeploymentQuery) FirstIDX(ctx context.Context) int {
...
func (_q *DeploymentQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *DeploymentQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *DeploymentQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *DeploymentQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *DeploymentQuery) WithOwner(opts ...func(*AgentQuery)) *DeploymentQuery {
	query := (&AgentClient{config: _q.config}).Query()
...
//		GroupBy(deployment.FieldPackageID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Deployment.Query().
//		Select(deployment.FieldPackageID).
//		Scan(ctx, &v)
...
func (_q *DeploymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Deployment, error) {
	...
	if query := _q.withOwner; query != nil {
		if err := _q.loadOwner(ctx, query, nodes, nil,
			func(n *Deployment, e *Agent) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DeploymentQuery) loadOwner(ctx context.Context, query *AgentQuery, nodes []*Deployment, init func(*Deployment), assign func(*Deployment, *Agent)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Deployment)
	for i := range nodes {
		if nodes[i].agent_deployments == nil {
			continue
		}
		fk := *nodes[i].agent_deployments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "agent_deployments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
